...
@Component({
  ...
  template: `
    <div class="max-width-container py-12 sm:py-20 animate-in fade-in duration-700">
        ...
        <div class="flex flex-wrap items-center justify-center sm:justify-start gap-5 mb-8">
          ...
          <div>
            ...
            <p class="text-lg text-base-content/50 m-0 mt-2 font-medium leading-relaxed">
              ...
              <code
                class="bg-primary/10 px-2 py-0.5 rounded-lg text-primary font-bold border border-primary/10"
                >@angular-helpers/security/signal-forms</code
              >
            </p>
          </div>
        </div>
        ...
        <form class="space-y-8">
          ...
          <div class="space-y-2">
            ...
            @if (f.password().pending()) {
              <p
                class="text-[11px] text-info font-black uppercase tracking-widest mt-2 flex items-center gap-2"
                data-testid="sf-password-pending"
              >
          ...
          >
            <div class="flex gap-6">
              <div class="flex flex-col">
                <span class="text-[10px] text-base-content/30 uppercase font-black tracking-widest"
                  >Valid</span
                >
                <span
                  class="font-black text-sm"
                  [class.text-success]="f().valid()"
                  [class.text-error]="!f().valid()"
                  >{{ f().valid() ? 'YES' : 'NO' }}</span
                >
              </div>
              <div class="flex flex-col">
                <span class="text-[10px] text-base-content/30 uppercase font-black tracking-widest"
                  >Pending</span
                >
                <span class="font-black text-sm text-info">{{
                  f().pending() ? 'SYNC...' : 'DONE'
                }}</span>
              </div>
            </div>
          ...
          <div
            class="mt-8 p-6 bg-base-content/5 rounded-2xl border border-base-content/5 shadow-inner"
          >
            <span
              class="text-[10px] text-base-content/30 uppercase font-black block mb-4 tracking-widest"
              >Real-time Model State</span
            >
            <pre
      ...
      <div
        class="mt-16 p-8 bg-primary/5 rounded-[2.5rem] border border-primary/10 max-w-3xl mx-auto shadow-2xl"
      >
        ...
        <p class="text-sm text-base-content/50 leading-relaxed font-medium">
          This demo utilizes the cutting-edge
          <strong class="text-base-content">Angular Signal Forms API</strong>. Password validation
          is performed via a composite strategy: synchronous entropy analysis (<code
            class="text-primary font-bold"
            >strongPassword</code
          >) and asynchronous k-anonymity breach verification (<code class="text-accent font-bold"
            >hibpPassword</code
          >).
        </p>
      </div>
