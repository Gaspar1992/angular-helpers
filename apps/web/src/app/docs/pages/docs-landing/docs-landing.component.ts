...
import { PACKAGES } from '../../../core/config/packages.data';
  ...
  template: `
    <div class="max-width-container py-12 sm:py-20 animate-in fade-in duration-700">
      ...
      >
        ...
        @for (pkg of packages; track pkg.npmPackage) {
          <a
          ...
          >
            <div class="flex flex-col gap-4 items-start">
              ...
              <div class="flex flex-wrap gap-2">
                @if (pkg.badge) {
                  <span class="badge badge-primary font-bold">{{ pkg.badge }}</span>
                }
                @for (tag of pkg.tags; track tag) {
                  <span class="badge font-bold" [class.badge-primary]="$first && !pkg.badge">{{
                    tag
                  }}</span>
                }
              </div>
            ...
            <div
              class="bg-base-content/5 border border-base-content/5 rounded-xl px-4 py-3 mt-auto"
            >
