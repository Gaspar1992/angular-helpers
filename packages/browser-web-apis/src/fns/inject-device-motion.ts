...
export function injectDeviceMotion(): DeviceMotionRef {
  assertInInjectionContext(injectDeviceMotion);
  const destroyRef = inject(DestroyRef);
  const platformId = inject(PLATFORM_ID);
  const isBrowser = isPlatformBrowser(platformId);
  const service = inject(DeviceMotionService);

  const isSupported = signal<boolean>(false);
  const data = signal<DeviceMotionData | null>(null);
