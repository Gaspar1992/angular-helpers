...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-10 min-h-screen bg-base-100/50">
      <!-- Premium Header -->
      <header class="mb-10 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-6">
        <div class="flex items-center gap-4">
          <div class="p-3 bg-gradient-to-br from-primary/20 to-secondary/20 rounded-2xl shadow-inner">
            <span class="text-4xl sm:text-5xl drop-shadow-md">🗺️</span>
          </div>
          <div>
            <h1 class="text-3xl sm:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent m-0">
              OpenLayers Demo
            </h1>
        ...
        <div class="flex flex-wrap justify-center sm:justify-end gap-2 max-w-[300px]">
          <span class="badge badge-primary badge-outline badge-md font-semibold">Map Component</span>
          <span class="badge badge-secondary badge-outline badge-md font-semibold">Tile Layers</span>
      ...
      <!-- Main Map Stage -->
      <div class="relative w-full h-[650px] rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-base-300/50 mb-8 bg-base-300">
        <ol-map
          [center]="center()"
          [zoom]="zoom()"
        ...
        >
          ...
          >
            @if (selectedCityInfo(); as info) {
              <div class="bg-base-100/95 backdrop-blur-md border border-base-300 rounded-xl shadow-xl p-4 min-w-[220px]">
        ...
        <div class="ol-demo-toolbar absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000]">
          ...
          @if (selectActive() && interactionService.hasSelection()) {
            <button class="ol-demo-toolbar__btn text-error hover:bg-error/10 hover:text-error" (click)="clearSelection()" aria-label="Clear selection">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          @if (drawActive()) {
            ...
            <div class="flex flex-row gap-1 items-center px-1">
              <button class="ol-demo-toolbar__btn ol-demo-toolbar__btn--sm" [class.ol-demo-toolbar__btn--active-secondary]="drawType() === 'Polygon'" (click)="onDrawTypeClick('Polygon')" title="Polygon">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l10 6v10l-10 6-10-6V8z" /></svg>
              </button>
              <button class="ol-demo-toolbar__btn ol-demo-toolbar__btn--sm" [class.ol-demo-toolbar__btn--active-secondary]="drawType() === 'LineString'" (click)="onDrawTypeClick('LineString')" title="Line">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 19L19 5" /></svg>
              </button>
              <button class="ol-demo-toolbar__btn ol-demo-toolbar__btn--sm" [class.ol-demo-toolbar__btn--active-secondary]="drawType() === 'Point'" (click)="onDrawTypeClick('Point')" title="Point">
              ...
              <button class="ol-demo-toolbar__btn ol-demo-toolbar__btn--sm" [class.ol-demo-toolbar__btn--active-secondary]="drawType() === 'Circle'" (click)="onDrawTypeClick('Circle')" title="Circle">
          ...
          @if (measureActive()) {
            <div class="ol-demo-toolbar__divider"></div>
            <div class="flex flex-row gap-1 items-center px-1">
              <button class="ol-demo-toolbar__btn ol-demo-toolbar__btn--sm" [class.ol-demo-toolbar__btn--active-secondary]="measureType() === 'Polygon'" (click)="onMeasureTypeClick('Polygon')" title="Area">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l10 6v10l-10 6-10-6V8z" /></svg> Area
              </button>
              <button class="ol-demo-toolbar__btn ol-demo-toolbar__btn--sm" [class.ol-demo-toolbar__btn--active-secondary]="measureType() === 'LineString'" (click)="onMeasureTypeClick('LineString')" title="Distance">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 19L19 5" /></svg> Dist
              </button>
            </div>
          }
        </div>
        ...
        @if (interactionService.selectionCount() > 0 || drawnCount() > 0) {
          <div class="absolute top-4 left-4 z-[1000] flex flex-col gap-2">
            @if (interactionService.selectionCount() > 0) {
              <div class="badge badge-primary shadow-md font-medium px-3 py-3 rounded-lg border-none backdrop-blur-md bg-primary/90 text-white">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-1.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>
                {{ interactionService.selectionCount() }} selected
              </div>
            }
            @if (drawnCount() > 0) {
              <div class="flex items-center gap-1 badge badge-secondary shadow-md font-medium px-3 py-3 rounded-lg border-none backdrop-blur-md bg-secondary/90 text-white">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-1.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
                {{ drawnCount() }} drawn
                <button class="ml-2 btn btn-xs btn-circle btn-ghost min-h-0 h-5 w-5 bg-black/20 hover:bg-black/40 border-none text-white" (click)="clearDrawnFeatures()" title="Clear drawings">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
          <div class="card bg-base-100 shadow-xl border border-base-200/50">
            <div class="card-body p-6">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                  <div class="flex justify-between items-start mb-2">
                    <div class="flex items-center gap-2">
                      <div class="p-2 rounded-lg" [class.bg-white/20]="activeLayerId() === 'cities'" [class.bg-primary/10]="activeLayerId() !== 'cities'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" [class.text-white]="activeLayerId() === 'cities'" [class.text-primary]="activeLayerId() !== 'cities'"><path d="M12 22s-8-4.5-8-11.8A8 8 0 0 1 12 2a8 8 0 0 1 8 8.2c0 7.3-8 11.8-8 11.8z"/><circle cx="12" cy="10" r="3"/></svg>
                      </div>
                      <span class="font-bold text-lg tracking-tight">Cities</span>
                    </div>
                    <label class="swap swap-rotate z-10 btn btn-circle btn-sm btn-ghost" [class.hover:bg-white/20]="activeLayerId() === 'cities'" (click)="$event.stopPropagation()">
                      <input type="checkbox" [checked]="layerVisibility()['cities']" (change)="toggleLayerVisibility('cities')" />
                      <svg class="swap-on fill-current w-5 h-5 opacity-90" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17M12,4.5C7,4.5 2.73,7.61 1,12C2.73,16.39 7,19.5 12,19.5C17,19.5 21.27,16.39 23,12C21.27,7.61 17,4.5 12,4.5Z"/></svg>
                      <svg class="swap-off fill-current w-5 h-5 opacity-50" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M11.83,9L15,12.16C15,12.11 15,12.05 15,12A3,3 0 0,0 12,9C11.94,9 11.89,9 11.83,9M7.53,9.8L9.08,11.35C9.03,11.56 9,11.77 9,12A3,3 0 0,0 12,15C12.22,15 12.44,14.97 12.65,14.92L14.2,16.47C13.53,16.8 12.79,17 12,17A5,5 0 0,1 7,12C7,11.21 7.2,10.47 7.53,9.8M2,4.27L4.28,6.55L4.73,7C3.08,8.3 1.78,10 1,12C2.73,16.39 7,19.5 12,19.5C13.55,19.5 15.03,19.2 16.38,18.66L16.81,19.08L19.73,22L21,20.73L3.27,3M12,7A5,5 0 0,1 17,12C17,12.64 16.87,13.26 16.64,13.82L19.57,16.75C21.07,15.5 22.27,13.86 23,12C21.27,7.61 17,4.5 12,4.5C10.6,4.5 9.26,4.75 8,5.2L10.17,7.35C10.74,7.13 11.35,7 12,7Z"/></svg>
                    </label>
                  </div>
                  <div class="mt-auto flex gap-2">
                    <button class="btn btn-sm flex-1 font-semibold rounded-lg shadow-sm" [class.btn-outline]="activeLayerId() !== 'cities'" [class.bg-white]="activeLayerId() === 'cities'" [class.text-primary]="activeLayerId() === 'cities'" [class.border-none]="activeLayerId() === 'cities'" [class.hover:bg-gray-100]="activeLayerId() === 'cities'" (click)="fitToCities(); $event.stopPropagation()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1"><path d="M15 3h6v6"></path><path d="M9 21H3v-6"></path><path d="M21 3l-7 7"></path><path d="M3 21l7-7"></path></svg>
                      Zoom
                    </button>
                    <button class="btn btn-sm flex-1 font-semibold rounded-lg shadow-sm" [class.btn-outline]="activeLayerId() !== 'cities'" [class.bg-white]="activeLayerId() === 'cities'" [class.text-primary]="activeLayerId() === 'cities'" [class.border-none]="activeLayerId() === 'cities'" [class.hover:bg-gray-100]="activeLayerId() === 'cities'" (click)="toggleLayerVisibility('heatmap'); $event.stopPropagation()">
                ...
                >
                  <div class="flex justify-between items-start mb-2">
                    <div class="flex items-center gap-2">
                      <div class="p-2 rounded-lg" [class.bg-black/10]="activeLayerId() === 'military'" [class.bg-warning/10]="activeLayerId() !== 'military'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" [class.text-amber-900]="activeLayerId() === 'military'" [class.text-warning]="activeLayerId() !== 'military'"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>
                      </div>
                      <span class="font-bold text-lg tracking-tight">Military <span class="opacity-70 text-sm ml-1 font-mono bg-black/10 px-1.5 py-0.5 rounded-md">{{ militaryFeatures().length }}</span></span>
                    </div>
                    <label class="swap swap-rotate z-10 btn btn-circle btn-sm btn-ghost" [class.hover:bg-black/10]="activeLayerId() === 'military'" (click)="$event.stopPropagation()">
                      <input type="checkbox" [checked]="layerVisibility()['military']" (change)="toggleLayerVisibility('military')" />
                      <svg class="swap-on fill-current w-5 h-5 opacity-90" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17M12,4.5C7,4.5 2.73,7.61 1,12C2.73,16.39 7,19.5 12,19.5C17,19.5 21.27,16.39 23,12C21.27,7.61 17,4.5 12,4.5Z"/></svg>
                      <svg class="swap-off fill-current w-5 h-5 opacity-50" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M11.83,9L15,12.16C15,12.11 15,12.05 15,12A3,3 0 0,0 12,9C11.94,9 11.89,9 11.83,9M7.53,9.8L9.08,11.35C9.03,11.56 9,11.77 9,12A3,3 0 0,0 12,15C12.22,15 12.44,14.97 12.65,14.92L14.2,16.47C13.53,16.8 12.79,17 12,17A5,5 0 0,1 7,12C7,11.21 7.2,10.47 7.53,9.8M2,4.27L4.28,6.55L4.73,7C3.08,8.3 1.78,10 1,12C2.73,16.39 7,19.5 12,19.5C13.55,19.5 15.03,19.2 16.38,18.66L16.81,19.08L19.73,22L21,20.73L3.27,3M12,7A5,5 0 0,1 17,12C17,12.64 16.87,13.26 16.64,13.82L19.57,16.75C21.07,15.5 22.27,13.86 23,12C21.27,7.61 17,4.5 12,4.5C10.6,4.5 9.26,4.75 8,5.2L10.17,7.35C10.74,7.13 11.35,7 12,7Z"/></svg>
                    </label>
                  </div>
                  <div class="mt-auto">
                    <button class="btn btn-sm w-full font-semibold rounded-lg shadow-sm" [disabled]="militaryFeatures().length === 0" [class.btn-outline]="activeLayerId() !== 'military'" [class.bg-white]="activeLayerId() === 'military'" [class.text-amber-700]="activeLayerId() === 'military'" [class.border-none]="activeLayerId() === 'military'" [class.hover:bg-amber-50]="activeLayerId() === 'military'" (click)="clearMilitary(); $event.stopPropagation()">
                ...
                <div 
                  class="rounded-2xl p-5 cursor-pointer transition-all duration-300 relative overflow-hidden group flex flex-col justify-between min-h-[140px]"
                  [class.shadow-[0_8px_30px_rgba(236,72,153,0.3)]]="activeLayerId() === 'drawn-features'"
                ...
                >
                  <div class="flex justify-between items-start mb-2">
                    <div class="flex items-center gap-2">
                      <div class="p-2 rounded-lg" [class.bg-white/20]="activeLayerId() === 'drawn-features'" [class.bg-secondary/10]="activeLayerId() !== 'drawn-features'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" [class.text-white]="activeLayerId() === 'drawn-features'" [class.text-secondary]="activeLayerId() !== 'drawn-features'"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
                      </div>
                      <span class="font-bold text-lg tracking-tight">Drawn <span class="opacity-70 text-sm ml-1 font-mono bg-black/10 px-1.5 py-0.5 rounded-md text-white/90">{{ drawnCount() }}</span></span>
                    </div>
                    <label class="swap swap-rotate z-10 btn btn-circle btn-sm btn-ghost" [class.hover:bg-white/20]="activeLayerId() === 'drawn-features'" (click)="$event.stopPropagation()">
                      <input type="checkbox" [checked]="layerVisibility()['drawn-features']" (change)="toggleLayerVisibility('drawn-features')" />
                      <svg class="swap-on fill-current w-5 h-5 opacity-90" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17M12,4.5C7,4.5 2.73,7.61 1,12C2.73,16.39 7,19.5 12,19.5C17,19.5 21.27,16.39 23,12C21.27,7.61 17,4.5 12,4.5Z"/></svg>
                      <svg class="swap-off fill-current w-5 h-5 opacity-50" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M11.83,9L15,12.16C15,12.11 15,12.05 15,12A3,3 0 0,0 12,9C11.94,9 11.89,9 11.83,9M7.53,9.8L9.08,11.35C9.03,11.56 9,11.77 9,12A3,3 0 0,0 12,15C12.22,15 12.44,14.97 12.65,14.92L14.2,16.47C13.53,16.8 12.79,17 12,17A5,5 0 0,1 7,12C7,11.21 7.2,10.47 7.53,9.8M2,4.27L4.28,6.55L4.73,7C3.08,8.3 1.78,10 1,12C2.73,16.39 7,19.5 12,19.5C13.55,19.5 15.03,19.2 16.38,18.66L16.81,19.08L19.73,22L21,20.73L3.27,3M12,7A5,5 0 0,1 17,12C17,12.64 16.87,13.26 16.64,13.82L19.57,16.75C21.07,15.5 22.27,13.86 23,12C21.27,7.61 17,4.5 12,4.5C10.6,4.5 9.26,4.75 8,5.2L10.17,7.35C10.74,7.13 11.35,7 12,7Z"/></svg>
                    </label>
                  </div>
                  <div class="mt-auto">
                    <button class="btn btn-sm w-full font-semibold rounded-lg shadow-sm" [disabled]="drawnCount() === 0" [class.btn-outline]="activeLayerId() !== 'drawn-features'" [class.bg-white]="activeLayerId() === 'drawn-features'" [class.text-secondary]="activeLayerId() === 'drawn-features'" [class.border-none]="activeLayerId() === 'drawn-features'" [class.hover:bg-pink-50]="activeLayerId() === 'drawn-features'" (click)="clearDrawnFeatures(); $event.stopPropagation()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                      Clear All
                    </button>
                  </div>
            ...
            <div class="bg-base-200/40 backdrop-blur-md p-6 border-t border-base-200/80 min-h-[140px] flex flex-col justify-center rounded-b-xl shadow-inner">
              @if (activeLayerId() === 'military') {
                <div class="animation-fade-in">
                  <h3 class="text-sm font-bold text-base-content/60 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-warning"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
                    Tactical Tools
                  </h3>
                  <div class="flex flex-wrap gap-3">
                    <button class="btn btn-warning shadow-md hover:shadow-lg transition-all" (click)="addRandomSymbol()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                      Symbol
                    </button>
                    <button class="btn btn-outline border-warning/50 hover:border-warning hover:bg-warning/10 text-base-content" (click)="addEllipse()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1 text-warning"><ellipse cx="12" cy="12" rx="10" ry="6"></ellipse></svg>
                      Ellipse
                    </button>
                    <button class="btn btn-outline border-warning/50 hover:border-warning hover:bg-warning/10 text-base-content" (click)="addSector()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1 text-warning"><path d="M21.21 15.89A10 10 0 1 1 8 2.83"></path><path d="M22 12A10 10 0 0 0 12 2v10z"></path></svg>
                      Sector
                    </button>
                    <button class="btn btn-outline border-warning/50 hover:border-warning hover:bg-warning/10 text-base-content" (click)="addDonut()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1 text-warning"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="4"></circle></svg>
                      Donut
                    </button>
                  </div>
                </div>
              ...
              @if (activeLayerId() === 'cities') {
                <div class="animation-fade-in">
                  <h3 class="text-sm font-bold text-base-content/60 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-primary"><polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"></polygon><line x1="9" y1="3" x2="9" y2="18"></line><line x1="15" y1="6" x2="15" y2="21"></line></svg>
                    Quick Navigation
                  </h3>
                  <div class="flex flex-wrap gap-3">
                    <button class="btn btn-outline border-primary/40 hover:border-primary hover:bg-primary/10 text-base-content shadow-sm" (click)="jumpTo([2.17, 41.38], 12)">Barcelona</button>
                    <button class="btn btn-outline border-primary/40 hover:border-primary hover:bg-primary/10 text-base-content shadow-sm" (click)="jumpTo([-3.7, 40.42], 12)">Madrid</button>
                    <button class="btn btn-outline border-primary/40 hover:border-primary hover:bg-primary/10 text-base-content shadow-sm" (click)="jumpTo([-0.38, 39.47], 12)">Valencia</button>
                    <button class="btn btn-outline border-primary/40 hover:border-primary hover:bg-primary/10 text-base-content shadow-sm" (click)="jumpTo([-5.98, 37.39], 12)">Sevilla</button>
                    <div class="divider divider-horizontal mx-1"></div>
                    <button class="btn btn-primary shadow-md hover:shadow-lg" (click)="openRandomCityComponentPopup()">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1"><circle cx="12" cy="12" r="10"></circle><path d="M12 16v-4"></path><path d="M12 8h.01"></path></svg>
                      Random Popup
                    </button>
                  </div>
                </div>
              ...
              @if (activeLayerId() === 'drawn-features') {
                <div class="animation-fade-in text-center py-6">
                  <p class="text-base-content/70 flex items-center justify-center gap-3 font-medium">
                    <span class="p-2 bg-secondary/10 rounded-full">
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-secondary"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                    </span>
        ...
        <div class="flex flex-col gap-6">
          <div class="card bg-base-100 shadow-xl border border-base-200/50 h-full">
            <div class="card-body p-6">
              <h2 class="card-title text-xl font-bold flex items-center gap-2 mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-info"><path d="M2 12h4l2-9 5 18 3-10 5 3"></path></svg>
                Telemetry
              </h2>
              ...
              <div class="stats stats-vertical shadow-sm border border-base-200/50 w-full mb-4">
                ...
                <div class="stat py-3">
                  ...
                  <div class="stat-value text-lg font-mono tracking-tight text-info mt-1">{{ zoom() | number:'1.1-2' }}</div>
                </div>
              </div>

              <h3 class="text-sm font-bold text-base-content/60 uppercase tracking-wider mb-2 mt-4">Pointer Data</h3>
              <div class="bg-base-200 rounded-xl p-4 border border-base-300 font-mono text-sm shadow-inner flex-grow">
                @if (lastClick(); as click) {
                  <div class="flex justify-between mb-2 pb-2 border-b border-base-300">
                    ...
                    <span class="font-bold text-success">{{ click.coordinate?.[0]?.toFixed(4) }}, {{ click.coordinate?.[1]?.toFixed(4) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-base-content/60">Screen X/Y:</span>
                    <span class="font-bold">{{ click.pixel?.[0] | number:'1.0-0' }}px, {{ click.pixel?.[1] | number:'1.0-0' }}px</span>
                  </div>
                } @else {
      ...
      <div class="mt-12 opacity-80 hover:opacity-100 transition-opacity duration-300">
        <h2 class="text-sm font-bold text-base-content/60 uppercase tracking-wider mb-4 ml-2">Modular Architecture</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="bg-gradient-to-r from-base-200 to-base-100 rounded-xl p-4 border-l-4 border-l-primary shadow-sm">
            <code class="text-primary font-bold text-sm bg-primary/10 px-2 py-1 rounded">@angular-helpers/openlayers/core</code>
            <p class="text-base-content/70 text-xs mt-2 font-medium">Map component, services, base types</p>
          </div>
          <div class="bg-gradient-to-r from-base-200 to-base-100 rounded-xl p-4 border-l-4 border-l-secondary shadow-sm">
            <code class="text-secondary font-bold text-sm bg-secondary/10 px-2 py-1 rounded">@angular-helpers/openlayers/layers</code>
            <p class="text-base-content/70 text-xs mt-2 font-medium">Tile, vector, image layer components</p>
          </div>
          <div class="bg-gradient-to-r from-base-200 to-base-100 rounded-xl p-4 border-l-4 border-l-accent shadow-sm">
            <code class="text-accent font-bold text-sm bg-accent/10 px-2 py-1 rounded">@angular-helpers/openlayers/controls</code>
            <p class="text-base-content/70 text-xs mt-2 font-medium">Zoom, attribution, scale, fullscreen</p>
          </div>
        </div>
      </div>
  `,
