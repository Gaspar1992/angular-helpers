...
export default {
  ...
  },
  {
    ...
    scope: 'root',
    importPath: '@angular-helpers/worker-http/realtime',
    requiresSecureContext: false,
    browserSupport: 'All browsers with Web Workers support',
    notes: [
      'WorkerWebSocketClient and WorkerSseClient run off-main-thread',
      'Supports automated heartbeats and exponential backoff reconnection with jitter',
      'Correlation ID auto-matching for Promise-based request() calls',
      'Works with standard Workers or MessagePorts (for SharedWorker configurations)',
    ],
    methods: [
      {
        name: 'attachRealtimeWorker',
        signature: 'attachRealtimeWorker(globalScope?: any): void',
        description:
          'Worker-side message handler that attaches listeners to manage WebSocket and SSE connections. Call this inside your web worker file.',
        returns: 'void',
      },
      {
        name: 'WorkerWebSocketClient',
        signature:
          'constructor(worker: Worker | MessagePort, config: WebSocketClientConfig, destroyRef?: DestroyRef)',
        description:
          'Client for managing WebSocket connections off-main-thread. Exposes status() signal and messages$ stream.',
        returns: 'WorkerWebSocketClient',
      },
      {
        name: 'WorkerSseClient',
        signature:
          'constructor(worker: Worker | MessagePort, config: SseClientConfig, destroyRef?: DestroyRef)',
        description:
          'Client for managing Server-Sent Events (EventSource) connections off-main-thread. Exposes status() signal and events$ stream.',
        returns: 'WorkerSseClient',
      },
      {
        name: 'send',
        signature: 'send(message: WebSocketMessage): void',
        description:
          'Sends a JSON-serialized WebSocket message from the main thread via the worker.',
        returns: 'void',
      },
      {
        name: 'sendRaw',
        signature: 'sendRaw(data: string): void',
        description: 'Sends a raw text WebSocket message from the main thread via the worker.',
        returns: 'void',
      },
      {
        name: 'request',
        signature: 'request(type: string, data: any, opts?: WebSocketRequestOptions): Promise<any>',
        description:
          'Sends a request message and waits for a response message with matching correlationId. Rejects on timeout.',
        returns: 'Promise<any>',
      },
    ],
    example: `import { WorkerWebSocketClient, WorkerSseClient } from '@angular-helpers/worker-http/realtime';

const worker = new Worker(new URL('./realtime.worker', import.meta.url), { type: 'module' });
...
// Observe connection status signal
const status = wsClient.status;
console.log('Status:', status().state);

// Subscribe to messages
wsClient.messages$.subscribe(msg => console.log('Message:', msg.data));

// Correlate request/response
const response = await wsClient.request('get_balance', { accountId: '123' });
console.log('Balance:', response);

// 2. SSE Client
const sseClient = new WorkerSseClient(worker, {
  url: 'https://api.example.com/events',
  events: ['ticker-update'],
});

sseClient.events$.subscribe(e => console.log('Event:', e.event, e.data));`,
  },
];
